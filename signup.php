<?php
include("dataconnection.php");

use PHPMailer\PHPMailer\PHPMailer;
use PHPMailer\PHPMailer\SMTP;
use PHPMailer\PHPMailer\Exception;

require 'phpmailer/src/Exception.php';
require 'phpmailer/src/PHPMailer.php';
require 'phpmailer/src/SMTP.php';

if (isset($_POST['submit'])) {
  $ic = $_POST['ic'];
  $name = $_POST['name'];
  $email = $_POST['email'];
  $pass = $_POST['password'];
  $confirm_pass = $_POST['confirm_pass'];
  $type = $_POST['user_type'];
  $code = mysqli_real_escape_string($con, md5(rand()));

  if (mysqli_num_rows(mysqli_query($con, "SELECT * FROM register WHERE email='{$email}'")) > 0) {
    $msg = "<div class='alert alert-danger'>{$email} - This email address has been already exists.</div>";
  }
  elseif (mysqli_num_rows(mysqli_query($con, "SELECT * FROM register WHERE ic='{$ic}'")) > 0) {
    $msg = "<div class='alert alert-danger'>{$ic} - This ic has been already exists.</div>";
  }elseif ($pass!=$confirm_pass) {
    $msg = "<div class='alert alert-danger'>{$ic} - Password not match.</div>";
  }
  else {
    $sql = "INSERT INTO register (ic,name,email,password,confirm_pass,user_type,code)VALUES ('$ic','$name','$email','$pass','$confirm_pass','$type','$code')";
    $rs = mysqli_query($con, $sql);

    if (isset($_POST['submit'])) {
        //Create an instance; passing `true` enables exceptions
        $mail = new PHPMailer(true);

        try {
            //Enable verbose debug output
            $mail->isSMTP();                                            //Send using SMTP
            $mail->Host       = 'smtp.gmail.com';                     //Set the SMTP server to send through
            $mail->SMTPAuth   = true;                                   //Enable SMTP authentication
            $mail->Username   = 'judsonchin@gmail.com';                     //SMTP username
            $mail->Password   = 'ridmmmvckrkhjymw';                               //SMTP password
            $mail->SMTPSecure = 'ssl';                                   //Enable implicit TLS encryption
            $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

            //Recipients
            $mail->setFrom('judsonchin@gmail.com');
            $mail->addAddress($_POST['email']);

            //Content
            $mail->isHTML(true);       //Set email format to HTML
            $mail->Subject = 'Verification Signup Email';
            $mail->Body    = 'Here is the verification link <b><a href="http://localhost/Car%20Rental/login.php?verification='.$code.'">http://localhost/Car%20Rental/login.php?verification='.$code.'</a></b>';

            $mail->send();
            // echo 'Message has been sent';
        } catch (Exception $e) {
            // echo "Message could not be sent. Mailer Error: {$mail->ErrorInfo}";
        }
        echo "</div>";
        $msg = "<div class='alert alert-info'>We've send a verification link on your email address.</div>";
    }
  }
}
?>

<html lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title>Car Rental</title>
<!-- bootstrap -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-GLhlTQ8iRABdZLl6O3oVMWSktQOp6b7In1Zl3/Jr59b6EGGoI1aFkw7cmDA6j6gD" crossorigin="anonymous">
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/js/bootstrap.bundle.min.js" integrity="sha384-w76AqPfDkMBDXo30jS1Sgez6pr3x5MlQ1ZAGC+nuZB+EYdgRZgiwxhTBTkF7CXvN" crossorigin="anonymous"></script>
<!-- css file -->
    <link rel="stylesheet" href="signup.css">
<!-- js file -->
    <script src="signup.js" charset="utf-8"></script>
  </head>
  <body>
    <section id="navbar">
      <nav class="navbar navbar-expand-lg navbar-dark p-md-3" style="background-color:black;">
        <div class="container-fluid">
          <a class="navbar-brand mx-auto" href="index.html" style="font-size:40px;font-weight:bold;color:white;"><span style="color: #6E4BE7;">Jo</span>Car</a>
          <!-- <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNavDropdown" aria-controls="navbarNavDropdown" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
          </button>
          <div class="collapse navbar-collapse" id="navbarNavDropdown">
            <ul class="navbar-nav ms-auto">
              <li class="nav-item">
                <a class="nav-link" href="index.html" style="color:white;font-size:20px;margin-left:20px;">Home</a>
              </li>
            </ul>
          </div> -->
        </div>
      </nav>
    </section>
    <section id="form">
      <h1 class="form-title">Sign Up</h1>
      <div class="container">
        <div class="Cart">
          <div class="row">
            <div class="col-lg-12">
              <p style="text-align:center">Sign up with your information</p>
              <?php echo $msg; ?>
              <form class="" action="signup.php" method="post">
                <div class="form-group col-lg-12">
                  <input type="text" class="form-control" name="ic" value="" placeholder="Enter Your IC No" required><br>
                </div>
                <div class="form-group col-lg-12">
                  <input type="text" class="form-control" name="name" value="" placeholder="Enter Your Name" required><br>
                </div>
                <div class="form-group col-lg-12">
                  <input type="email" class="form-control" name="email" value="" placeholder="Enter Your Email" required><br>
                </div>
                <div class="form-group col-lg-12">
                  <input type="password" class="form-control" name="password" value="" placeholder="Enter Your Password" pattern="[A-Za-z0-9]{8}" onclick="pass_attention()" required><br>
                </div>
                <div class="form-group col-lg-12">
                  <input type="password" class="form-control" name="confirm_pass" value="" placeholder="Confirm Your Password" pattern="[A-Za-z0-9]{8}" required><br>
                </div>
                <div class="form-group col-lg-12">
                  <!-- <label for="user_type">Enter Your Identity:</label> -->
                  <select class="form-control" name="user_type">
                      <option value = "user">User</option>
                      <!-- <option value = "admin">admin</option> -->
                    </optgroup>
                  </select>
                </div>
                <div class="btn col-lg-12">
                  <br>
                  <button type="submit" class="block btn btn-outline-success" name="submit">Sign Up</button>
                </div>
              </form>
              <div class="col-lg-12">
                <p style="text-align:center">Already have an account? <a href="login.php">Login</a></p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  </body>
</html>
