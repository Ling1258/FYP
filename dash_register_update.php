<?php
  ...
  if (isset($_GET['ID'])){
    ...
    $user_type=$_POST['user_type'];
	  ...
	  // } else{
		// 	mysqli_query($con,"update register set ic='$ic', name='$name', email='$email', password='$password', confirm_pass='$confirm_pass', user_type='$user_type' where id='$id'");
		// 	header("Location: dash_register_list.php");
		// }
    ...
    $ic_check_query = "SELECT * FROM register WHERE ic='$ic' AND id!='$id'";
		...
		if (mysqli_num_rows($ic_check_result) > 0) {
		    ...
		    $msg = "<div class='alert alert-danger'>Password not match.</div>";
				...
				mysqli_query($con, "update register set ic='$ic', name='$name', email='$new_email', password='$password', confirm_pass='$confirm_pass', user_type='$user_type' where id='$id'");
				mysqli_query($con, "UPDATE partnership SET p_email='$new_email' WHERE p_email='$email'");
				mysqli_query($con, "UPDATE rent_record SET driver_email='$new_email' WHERE driver_email='$email'");
				header("Location: dash_register_list.php");
			}
		}
  }
